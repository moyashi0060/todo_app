...
### `is_completed` へのインデックス設計

フィルタリングの主軸となる `is_completed` カラムに `index=True` を付与している。
...
**インデックスなしの場合**: `WHERE is_completed = true` はテーブル全件を走査する O(n) のフルテーブルスキャンになる。

**インデックスありの場合**: B-tree インデックスにより O(log n) で該当行に到達できる。

`is_completed` は `true/false` の2値しか取らず、カーディナリティが低い（インデックス効果が薄いという議論もある）。しかし今回の用途はフィルタリングの主目的がこのカラムであるため、タスク数が増えた際にフルスキャンを避ける意図で付与した。

### タスク一覧の ORDER BY 設計

`GET /tasks` は `ORDER BY created_at ASC` で返却順序を保証している。

SQL は ORDER BY を指定しない場合、返却順序を仕様上保証しない。ORDER BY なしでは、タスク数が増えた際や DB の内部最適化によって表示順が変わる可能性がある。

| ソート基準 | メリット | デメリット |
|---|---|---|
| `created_at ASC`（採用） | 追加順で直感的、要件の「作成日時が付与される」と整合 | 同一時刻の場合に順序不定（実用上は問題なし） |
| `id ASC` | シンプル、高速 | 削除後に番号が飛ぶため視覚的に不連続 |
| ソートなし | 実装コスト最小 | 返却順序が不定、テストが困難 |

要件に「作成日時が自動付与される」と明記されているため、`created_at ASC` が最も整合する。
